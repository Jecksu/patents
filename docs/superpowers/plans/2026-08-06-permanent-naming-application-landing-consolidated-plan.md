...
- 步骤 5 形式要求：摘要 298 字（上限 300）；权利要求 1 至 58 编号连续（2026-08-09 在权利要求 38 之后新增降级限定项 39，其后各项顺延一位），无前向引用与自引用；权利要求 53 为系统项、54 至 56 依附 53、57 为电子设备、58 为介质，57 与 58 引用的“权利要求1至52”与方法项区间（1 项独权加 2 至 52 从属）完全一致；附图说明声明四幅，四条“图N为本发明实施例提供的……”句式逐字包含生成器 `figure_titles` 的对应图题；`rg -n "图[1-9]" | rg -v "图1|图2|图3|图4"` 退出码 1，无图5及旧主题引用。
...
**步骤 4（重编号与引用）：** 全书共 58 项（2026-08-09 新增降级限定项 39 后）。校验脚本输出“编号连续: True 总项数: 58”，无前向或自引用输出。多引用仅出现在权利要求 45 至 47 引用权利要求 44，均为单项引用；权利要求 57、58 引用范围为“权利要求1至52中任一项”。
...
**步骤 3（装置与介质）：** 权利要求 57 电子设备、权利要求 58 计算机可读存储介质的方法引用范围为“权利要求1至52中任一项”；权利要求 58 另行限定其程序执行时完成权利要求 1 的七个步骤、仅在 CLOSURE_PASS 成立后分类、同批原子发布 CAD_DIFF_EVENT_V1，并明确不建立独立于 R_seed、R_11、冻结集合、CLOSURE_PASS 和该事件的处理主线。

**步骤 4（单一性与类别一致）：** 四个类别为方法（1 至 52）、系统（53 至 56）、电子设备（57）、计算机可读存储介质（58），共享同一七步主线这一相同或相应的特定技术特征；系统模块与七步一一对应，无“自动生成永久名称”或“闭包前判定新增／删除”的功能；介质权利要求未另造独立处理主线。
