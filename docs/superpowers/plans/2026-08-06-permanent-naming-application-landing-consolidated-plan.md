...
**步骤 4（有益效果）：** 改为 7 个编号项，每项按技术特征、作用机理、技术效果三段式表述，覆盖锚定文档第 5.3 节六条因果链加候选规模上界。效果用语限定为降低错误身份传播风险、给出候选规模上界、避免把证据不足端点提前归入增删、保持结论可复算可追溯；未使用显著、大幅、提高效率、降低计算复杂度等表述，也未给出真实性能百分比。
